...
import { UserCheck, Plus, Pencil, Trash2, Briefcase, BadgeCheck, Loader2 } from "lucide-react";
...
export default function InstructorManagement() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async (res) => {
      if (res.success) {
        // Note: keeping target query key as "testimonials" if that matches your backend architecture
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-gray-900 p-5 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-xs">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-cyan-500/10 rounded-2xl hidden sm:block">
            <UserCheck className="text-cyan-600 dark:text-cyan-400" size={28} />
          </div>
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-gray-100 tracking-tight uppercase">
              Instructors <span className="text-cyan-600 dark:text-cyan-400">Panel</span> 🗣️
            </h1>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Control, upgrade and manage your team of instructors.</p>
          </div>
        </div>
        
        <Button
          onClick={() => { resetForm(); setIsModalOpen(true); }}
          className="w-full sm:w-auto bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl font-bold text-xs uppercase tracking-wider h-11 shadow-xs active:scale-95"
        >
          <Plus size={16} className="mr-1" /> Add Instructors
        </Button>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-2">
          ...
          <p className="text-sm font-medium text-cyan-500/70 animate-pulse tracking-wider uppercase">Loading Instructors...</p>
        </div>
      ) : isError ? (
        <div className="text-center py-16 bg-white dark:bg-gray-900 border border-red-100 dark:border-red-950/40 rounded-3xl text-red-500">
          <p className="text-sm font-bold uppercase tracking-wide">⚠️ Failed to load Instructors data!</p>
        </div>
      ) : (
        <>
                ...
                >
                  <div className="flex gap-4 items-center">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs font-semibold text-gray-500 mt-1 flex items-center gap-1">
                        <Briefcase size={12} className="text-gray-400" /> {inst.position?.title || "N/A"}
                      </p>
                    </div>
                  </div>
                  ...
                  {inst.items && inst.items.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-2 border-t border-gray-50 dark:border-gray-800/50">
                      {inst.items.slice(0, 4).map((item, i) => (
                        <span key={`skill-mob-${i}`} className="bg-cyan-50 dark:bg-cyan-900/20 text-cyan-700 dark:text-cyan-400 border border-cyan-100 dark:border-cyan-900/30 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wide">
                          {item}
                        </span>
                      ))}
                      {inst.items.length > 4 && (
                  ...
                  <div className="flex gap-2 pt-2 border-t border-gray-50 dark:border-gray-800/50">
                    <Button
                      ...
                      className="flex-1 h-9 rounded-xl bg-yellow-500/5 text-yellow-600 dark:text-yellow-400 hover:bg-yellow-600 hover:text-white text-xs font-bold uppercase tracking-wider"
                    >
                    ...
                    <Button
                      ...
                      className="flex-1 h-9 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-600 hover:text-white text-xs font-bold uppercase tracking-wider"
                    >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-50/50 dark:bg-gray-800/40 border-b border-gray-100 dark:border-gray-800 font-bold">
                  <tr>
                    <th className="p-5 font-black text-gray-500 uppercase text-[10px] tracking-widest">Image</th>
                    <th className="p-5 font-black text-gray-500 uppercase text-[10px] tracking-widest">Name</th>
                    <th className="p-5 font-black text-gray-500 uppercase text-[10px] tracking-widest">Position</th>
                    <th className="p-5 font-black text-gray-500 uppercase text-[10px] tracking-widest">Skills Matrix</th>
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                  {instructors.length === 0 ? (
                    ...
                    instructors.map((inst, index) => (
                      <tr key={inst.id || `inst-row-${index}`} className="hover:bg-gray-50/40 dark:hover:bg-gray-800/20 transition-all group">
                        <td className="p-5">
                          {inst.image ? (
                        ...
                        <td className="p-5">
                          <div className="flex flex-wrap gap-1.5 max-w-xs">
                            {inst.items?.slice(0, 3).map((item, i) => (
                              <span key={`skill-dsktp-${i}`} className="bg-cyan-50 dark:bg-cyan-900/20 text-cyan-700 dark:text-cyan-400 border border-cyan-100 dark:border-cyan-900/30 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider">
                                {item}
                              </span>
                            ))}
                            {inst.items?.length > 3 && (
  ...
  return (
    <div className="text-center py-20 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl text-gray-400 dark:text-gray-600 italic shadow-xs w-full">
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-sm font-semibold tracking-wide uppercase mt-2">No instructors listed</p>
        <p className="text-xs text-gray-400 not-italic">Click "Add Instructor" to expand your corporate team network.</p>
      </div>
    </div>
  );
}
